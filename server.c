...
void crerequest(int client_socket) {
    send(client_socket, "Mirror", 8, 0);
    ...
    while (1) {
        num_directories = 0;
        num=0;
        //send first msg if it server or Mirrors
...
if (send_directory_by_date(client_socket, home_dir) == -1) {
    ...
    fclose(file_list);
    //check the size of the file to determine if the files were found or not
   ...
   send(client_socket, "END_OF_FILE", strlen("END_OF_FILE"), 0);
    ...
    //search file with  matching extensions
...
void traverse_directory_collect_files(const char *dir_path, FILE *file_list) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (S_ISREG(st.st_mode) && st.st_size >= size1 && st.st_size <= size2) {
            // Append the file path to to the FILE File_list
            fprintf(file_list, "%s\n", path);
...
int compare_directories_by_creation_time(const void *a, const void *b) {
    ...
    //dir_info contains file creation time 
...
void handleMirror(int client_socket, char *client_ip){
    //Send which mirror it should connect
    printf("Client Disconnected: %s", client_ip);
    write(client_socket, "MIRROR1", 7);
    
}

void handleMirror2(int client_socket, char *client_ip){
    //send which mirror it should connect
    printf("Client Disconnected: %s", client_ip);
    write(client_socket, "MIRROR2", 7);
...
void start_server() {
    ...
    while (1) {
        ...
        if (pid == 0) {
            close(server_socket);
            if(conn > 3 && conn < 7){
                handleMirror(client_socket,client_ip);
                close(client_socket);
                exit(0);
            }
            
            else if(conn < 4){
            crerequest(client_socket);
        
            close(client_socket);
            printf("Client Disconnected:%s\n",client_ip);
            exit(0);
            }
            else if(conn > 6 && conn < 10){
                handleMirror2(client_socket,client_ip);
                close(client_socket);
                printf("Client Disconnected:%s\n",client_ip);
                exit(0);
            }
            else if(conn > 9){
                //if connection is more then 9 use modulos
                int func_index = (conn - 9) % 3;
            switch (func_index) {
                case 0:
                    crerequest(client_socket);
                    close(client_socket);
